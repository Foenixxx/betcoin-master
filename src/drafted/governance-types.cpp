...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "BetNetwork") type = BetNetwork;
    if(strType == "BetNetworkVariable") type = BetNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == BetNetwork) s = "BetNetwork";
    if(type == BetNetworkVariable) s = "BetNetworkVariable";
