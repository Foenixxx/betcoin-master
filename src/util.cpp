...
const char * const BITCOIN_CONF_FILENAME = "bet.conf";
const char * const BITCOIN_PID_FILENAME = "betd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "bet" is a composite category enabling all Bet-related debug output
            if(ptrCategory->count(string("bet"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BetCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BetCore
    // Mac: ~/Library/Application Support/BetCore
    // Unix: ~/.betcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BetCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BetCore";
#else
    // Unix
    return pathRet / ".betcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty bet.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override bet.conf
